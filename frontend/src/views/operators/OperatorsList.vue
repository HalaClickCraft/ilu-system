      ...
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6">
        ...
        <form @submit.prevent="createOperator" class="space-y-4">
          <div class="grid grid-cols-2 gap-3">
            ...
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Prenom</label><input v-model="form.firstName" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none" /></div>
          </div>
          <div class="grid grid-cols-2 gap-3">
            ...
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Role</label><input v-model="form.role" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="Ex: Operateur" /></div>
          </div>
          <div class="grid grid-cols-2 gap-3">
          ...
          <div class="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" :disabled="creating" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm rounded-lg transition">Creer</button>
          </div>
        </form>
      </div>
    </div>
...
const error = ref('')
const form = ref({ lastName: '', firstName: '', employeeId: '', role: '', hireDate: '', exitDate: '', projectId: '', zoneId: '', workstationId: '' })
...
const fetchProjects = async () => { try { projects.value = (await structureApi.getAll()).data } catch (e) { console.error(e) } }

const openCreateModal = async () => {
  form.value = { lastName: '', firstName: '', employeeId: '', role: '', hireDate: '', exitDate: '', projectId: '', zoneId: '', workstationId: '' }
  error.value = ''
...
const createOperator = async () => {
  ...
  try {
    ...
    showCreateModal.value = false; fetchOperators()
 } catch (e) { const msg = e.response?.data?.message || e.message || 'Erreur inconnue'; error.value = msg + ' (status: ' + (e.response?.status || 'n/a') + ')'; alert('Erreur: ' + error.value) } finally { creating.value = false } 
}
