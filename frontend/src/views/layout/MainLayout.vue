  ...
  <div class="min-h-screen flex bg-gray-100">
    <aside :class="[sidebarOpen ? 'w-64' : 'w-20']" class="bg-slate-900 text-white transition-all duration-300 flex flex-col fixed h-full z-30">
      ...
      <nav class="flex-1 py-4 space-y-1 px-3 overflow-y-auto">
        <!-- Dashboard -->
        <router-link v-if="!isDeptOnly" to="/" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors hover:bg-slate-800" :class="{ 'bg-emerald-600 hover:bg-emerald-700': $route.path === '/' }">
        ...
        <!-- Operators -->
        <router-link v-if="!isDeptOnly" to="/operators" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors hover:bg-slate-800" :class="{ 'bg-emerald-600 hover:bg-emerald-700': $route.path.startsWith('/operators') }">
          <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
          <span v-if="sidebarOpen">Operateurs</span>
        </router-link>
        <!-- Training -->
        <router-link v-if="!isDeptOnly" to="/training" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors hover:bg-slate-800" :class="{ 'bg-emerald-600 hover:bg-emerald-700': $route.path.startsWith('/training') }">
          <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>
          <span v-if="sidebarOpen">Formation</span>
        </router-link>
        <!-- Structure -->
        <router-link v-if="!isDeptOnly" to="/structure" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors hover:bg-slate-800" :class="{ 'bg-emerald-600 hover:bg-emerald-700': $route.path === '/structure' }">
        ...
        <!-- Teams -->
        <router-link v-if="!isDeptOnly" to="/teams" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors hover:bg-slate-800" :class="{ 'bg-emerald-600 hover:bg-emerald-700': $route.path === '/teams' }">
          <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>
          <span v-if="sidebarOpen">Equipes</span>
        </router-link>

        <!-- Onboarding - visible to everyone -->
        <router-link to="/onboarding" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors hover:bg-slate-800" :class="{ 'bg-emerald-600 hover:bg-emerald-700': $route.path.startsWith('/onboarding') }">
        ...
        <router-link v-if="canAccessEvaluation" to="/evaluation/sessions" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors hover:bg-slate-800" :class="{ 'bg-emerald-600 hover:bg-emerald-700': $route.path.startsWith('/evaluation/sessions') }">
          <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5-1a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
          <span v-if="sidebarOpen">Evaluation</span>
        </router-link>

        <router-link v-if="canAccessEvaluation" to="/evaluation/templates" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors hover:bg-slate-800" :class="{ 'bg-emerald-600 hover:bg-emerald-700': $route.path.startsWith('/evaluation/templates') }">
        ...
        <router-link v-if="canManageQuestions" to="/evaluation/questions" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors hover:bg-slate-800" :class="{ 'bg-emerald-600 hover:bg-emerald-700': $route.path.startsWith('/evaluation/questions') }">
          <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 20h9m-9-4h9m-9-4h9M3 20h.01M3 16h.01M3 12h.01M3 8h.01M3 4h.01"></path></svg>
...
const canAccessEvaluation = computed(() => authStore.hasAnyRole(['ADMIN', 'CHEF_EQUIPE', 'RESP_HSE', 'AGENT_QUALITE', 'RESP_QUALITE']))
