  ...
  <div class="space-y-6">
    ...
    <template v-else-if="formation">
      ...
      <div class="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
        <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm"><p class="text-xs text-gray-500">Operateur</p><p class="mt-1 text-sm font-semibold">{{ formation.operatorName }}</p></div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm"><p class="text-xs text-gray-500">Defauts total</p><p class="mt-1 text-sm font-semibold" :class="defectsColor">{{ totalDefects }} / {{ formation.qualityObjective ?? 7 }}</p></div>
        <div class="rounded-xl border border-purple-200 bg-purple-50 p-4 shadow-sm">
          ...
          <div v-if="canEditQuality && editingQO" class="mt-1 flex gap-1">
            <input v-model.number="editQOValue" min="1" type="number" class="w-14 rounded border border-purple-300 px-1 text-sm">
            <button @click="saveQualityObjective" class="text-xs text-purple-700">OK</button>
          </div>
          <div v-else class="mt-1 flex gap-2">
            <p class="text-sm font-semibold text-purple-700">{{ formation.qualityObjective ?? 7 }}</p>
            <button v-if="canEditQuality" @click="editingQO = true; editQOValue = formation.qualityObjective ?? 7" class="text-xs text-purple-600">Modifier</button>
          </div>
        </div>
      </div>
      ...
      <div class="rounded-xl border border-gray-200 bg-white shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 p-4">
          <div>
            ...
            <p class="mt-1 text-xs text-gray-500">Chef d'equipe : cadence - Agent qualite : defauts</p>
          </div>
          <div class="flex items-center gap-2">
            <span class="text-xs text-gray-400">{{ completeDays }} / 12 jours complets</span>
            <button v-if="canContribute && hasDirty" @click="saveAll" :disabled="saving" class="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs text-white disabled:opacity-50">Enregistrer</button>
            <button v-if="canContribute" @click="evaluate" :disabled="evaluating || !readyToEvaluate" class="rounded-lg bg-amber-500 px-3 py-1.5 text-xs text-white disabled:opacity-50">Auto-evaluer</button>
          </div>
        </div>
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead class="bg-gray-50"><tr>
              <th class="w-48 px-3 py-2 text-left text-xs font-medium text-gray-500">Indicateur</th>
              <th v-for="day in 12" :key="day" class="w-16 px-1 py-2 text-center text-xs font-medium">J{{ day }}</th>
            ...
            <tbody>
              <tr class="bg-teal-50/50">
                ...
                <td v-for="day in 12" :key="'target' + day" class="px-1 py-2 text-center text-xs text-teal-700">{{ formation.targetCadence ?? '-' }}</td>
                <td class="bg-gray-100 px-3 py-2 text-center text-xs font-bold">{{ formation.targetCadence ?? '-' }}</td>
              ...
              <tr>
                ...
                <td v-for="day in 12" :key="'cadence' + day" class="px-1 py-2 text-center">
                  <input v-if="canEditCadence && canEdit" v-model.number="dayData[day].cadence" min="0" type="number" class="w-14 rounded border border-emerald-300 py-1 text-center text-xs" @input="dirtyDays.add(day)" />
                  <span v-else :title="dayData[day].cadenceSubmittedBy ? 'Saisi par ' + dayData[day].cadenceSubmittedBy : ''" class="text-xs font-semibold" :class="getCadenceColor(dayData[day].cadence)">{{ dayData[day].cadence ?? '-' }}</span>
                </td>
                <td class="bg-gray-100 px-3 py-2 text-center text-xs font-bold" :class="cadenceColor">{{ averageCadence }}</td>
                <td class="bg-gray-100 px-3 py-2 text-center text-xs">-</td>
              </tr>
              <tr>
                ...
                <td v-for="day in 12" :key="'defects' + day" class="px-1 py-2 text-center">
                  <input v-if="canEditDefects && canEdit" v-model.number="dayData[day].defects" min="0" type="number" class="w-14 rounded border border-purple-300 py-1 text-center text-xs" @input="dirtyDays.add(day)" />
                  <span v-else :title="dayData[day].defectsSubmittedBy ? 'Saisi par ' + dayData[day].defectsSubmittedBy : ''" class="text-xs font-semibold" :class="(dayData[day].defects ?? 0) > 0 ? 'text-red-600' : 'text-emerald-600'">{{ dayData[day].defects ?? '-' }}</span>
                </td>
                <td class="bg-gray-100 px-3 py-2 text-center text-xs">-</td>
                <td class="bg-gray-100 px-3 py-2 text-center text-xs font-bold" :class="defectsColor">{{ totalDefects }}</td>
              </tr>
              <tr>
                <td colspan="14" class="bg-blue-50/30 px-3 py-2 text-xs text-blue-700">L'evaluation est automatique uniquement lorsque les deux mesures sont presentes pour les 12 jours.</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div v-if="formation.status === 'COMPLETED' && pendingEvalForThisOperator.length > 0" class="mt-4 mx-4 mb-4 bg-emerald-50 border border-emerald-300 rounded-xl p-5">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-sm text-emerald-700 mt-1">Cet operateur a reussi le suivi 12 jours. Lancez l'evaluation pour determiner le niveau de polyvalence.</p>
            </div>
            <button @click="goToEvaluation" class="bg-emerald-600 text-white px-5 py-2.5 rounded-lg hover:bg-emerald-700 font-medium">
        ...
        <div v-if="formation.status === 'COMPLETED' && pendingEvalForThisOperator.length === 0 && hasCheckedPending" class="mt-4 mx-4 mb-4 bg-blue-50 border border-blue-200 rounded-xl p-4">
          <p class="text-sm text-blue-700 font-medium">Evaluation deja effectuee pour cette formation.</p>
        </div>
      </div>
    </template>
  </div>
...
const statusLabel = status => ({ IN_PROGRESS: 'En cours', COMPLETED: 'Reussie', FAILED: 'Echouee' })[status] || status
...
const renderChart = () => {
  ...
  chartInstance = new ChartJS(chartCanvas.value, {
    ...
    data: {
      labels,
      datasets: [
        { label: 'Cadence Objectif', data: targetData, type: 'line', borderColor: '#f59e0b', borderDash: [5, 5], pointRadius: 0 },
        { label: 'Cadence Realisee', data: cadenceData, type: 'line', borderColor: '#10b981', backgroundColor: 'rgba(16,185,129,0.1)', fill: true, tension: 0.4, spanGaps: true },
        { label: 'Defauts', data: defectsData, type: 'bar', yAxisID: 'y1', borderColor: '#ef4444', backgroundColor: 'rgba(239,68,68,0.15)', spanGaps: true },
      ],
    },
    options: {
      responsive: true,
...
const refreshFormation = async () => {
  ...
  } catch (e) { console.error('Formation refresh error', e) }
}
  ...
  if (trackingRes.status === 'fulfilled') {
    for (const t of trackingRes.value.data) {
      if (t.dayNumber >= 1 && t.dayNumber <= 12) {
        dayData[t.dayNumber] = { cadence: t.actualCadence, defects: t.defects, cadenceSubmittedBy: t.cadenceSubmittedBy, defectsSubmittedBy: t.defectsSubmittedBy }
      }
    }
  } else {
...
const saveAll = async () => {
  ...
  try {
    ...
    for (const dayNumber of dirtyDays) {
      if (canEditCadence.value) dayData[dayNumber].cadenceSubmittedBy = authStore.user?.username || null
      if (canEditDefects.value) dayData[dayNumber].defectsSubmittedBy = authStore.user?.username || null
    }
    dirtyDays.clear()
...
const saveQualityObjective = async () => {
  try {
    ...
    error.value = requestError.response?.data?.message || "Impossible de mettre a jour l'objectif qualite."
  }
}
...
const resetFormation = async () => {
  try {
    ...
    error.value = requestError.response?.data?.message || "Impossible de reinitialiser la formation."
  }
}
